...
export const auditorPortalHandlers: StubHandler[] = [
  ...
  { method: 'POST', path: '/auth/login', body: (_p, payload) => {
    const body = (payload as { email?: string; token?: string }) || {};
    if (body.token) {
      return { token: `aud_${Math.random().toString(36).slice(2, 14)}`, redirectTo: '/auditor-portal' };
    }
    return { sent: true, email: body.email ?? null, expiresAt: new Date(Date.now() + 15 * 60_000).toISOString() };
  }},
