  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <PageHeader
        ...
        actions={
          <Button size="sm" leftIcon={<Plus className="h-4 w-4" />} onClick={() => setIsCreateOpen(true)}>
            Create Framework
          </Button>
        }
      />
      ...
      >
        <div className="space-y-4">
          <div>
            <Label htmlFor="fw-name" required>Framework Name</Label>
            <Input
              id="fw-name"
              value={formData.name}
          ...
          <div>
            <Label htmlFor="fw-type" required>Type</Label>
            <Input
              id="fw-type"
              value={formData.type}
...
function FrameworkCard({ framework }: { framework: Framework }) {
  ...
  const scoreColor = score >= 80 ? 'text-emerald-700' : score >= 50 ? 'text-yellow-700' : 'text-red-600';
        ...
        <CardBody density="comfy">
          ...
          <div className="mb-4">
            ...
            <div className="h-1.5 bg-surface-100 rounded-full overflow-hidden">
              <div className={cn('h-full transition-all', progressColor)} style={{ width: `${score}%` }} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-3 border-t border-surface-200">
            <div>
              ...
              <p className="text-small font-medium text-surface-800">{framework.requirementCount || 0}</p>
            </div>
            <div>
              <p className="text-xs text-surface-500">Mapped Controls</p>
              <p className="text-small font-medium text-surface-800">{framework.mappedControlCount || 0}</p>
            </div>
          </div>
