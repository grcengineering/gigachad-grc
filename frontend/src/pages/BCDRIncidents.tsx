...
import { AlertTriangle, Search, Activity, PlayCircle, CheckCircle2 } from 'lucide-react';
...
export default function BCDRIncidents() {
  ...
  const columns: DataTableColumn<BCDRIncident>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-surface-700">{formatDate(getDeclaredAt(row.original))}</span>
      ),
    },
    {
      id: 'resolvedAt',
      ...
      cell: ({ row }) => (
        <span className="text-surface-700">{formatDate(getResolvedAt(row.original))}</span>
      ),
    },
  ];

  const activeCount = stats?.active_count ?? incidents.filter((i) => i.status === 'active').length;
  const declaredCount =
    stats?.declared_count ?? incidents.filter((i) => i.status === 'declared').length;
  const inProgressCount =
    stats?.in_progress_count ??
    stats?.recovering_count ??
    incidents.filter((i) => i.status === 'in_progress' || i.status === 'recovering').length;
  const resolvedCount = stats?.resolved_this_month ?? stats?.resolved_count ?? 0;
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          tone="blue"
          onClick={() =>
            updateFilter('status', filters.status === 'in_progress' ? '' : 'in_progress')
          }
        />
        <StatCard
