...
export default function Contracts() {
  ...
  const columns: DataTableColumn<Contract>[] = [
    ...
    },
    {
      ...
      mobileLabel: 'Type',
      cell: ({ row }) => (
        <span className="text-surface-700 capitalize">{row.original.contractType.replace(/_/g, ' ')}</span>
      ),
    },
    {
      id: 'value',
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-surface-700">{new Date(row.original.endDate).toLocaleDateString()}</span>
      ),
    },
    {
      id: 'status',
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <PageHeader
        ...
        actions={
          <Button size="sm" leftIcon={<Plus className="h-4 w-4" />} onClick={() => navigate('/contracts/new')}>
            New Contract
          </Button>
        }
      />
