...
const STAT_TONES: StatCardTone[] = ['brand', 'accent', 'emerald', 'red', 'amber', 'blue', 'purple', 'neutral'];
  ...
  return (
    <div className="space-y-10 pb-16">
      <PageHeader
        ...
        meta={<Badge variant="brand" size="sm">v1</Badge>}
        actions={
          <>
      ...
      {/* ============ BUTTON ============ */}
      <Section title="Button" description="Use primary for the main action. Outline/ghost for secondary; danger for destructive.">
      ...
      {/* ============ BADGE ============ */}
      <Section title="Badge" description="Status pills. Always use dot for live status; omit dot for tags/labels.">
        <Subsection title="Variants">
          <div className="flex flex-wrap gap-2">
      ...
      {/* ============ FORMS ============ */}
      <Section title="Form controls" description="Input, Textarea, Select, Label, FieldHint — Headless UI under the hood.">
      ...
      {/* ============ CARD ============ */}
      <Section title="Card" description="The canonical container. Use slots (Header/Title/Description/Body/Footer) for structured cards.">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card interactive density="cozy">
            ...
            <p className="text-small text-surface-700 mt-1">
              Hover-lift + cursor-pointer when <code className="font-mono text-xs">interactive</code> is true.
            </p>
          </Card>
        </div>
      </Section>
      ...
      <Section title="StatCard &amp; Sparkline" description="At-a-glance metrics. tone= chooses the icon tint and sparkline color.">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Controls" value={142} delta={4.2} trend={TRENDS.up} tone="brand" caption="of 240 total" />
          <StatCard label="Findings" value={17} delta={-12} deltaPositiveIsGood={false} trend={TRENDS.down} tone="red" />
          <StatCard label="Coverage" value="86%" delta={0} trend={TRENDS.flat} tone="emerald" />
          <StatCard label="Vendors" value={31} trend={TRENDS.up} tone="amber" caption="3 expiring soon" />
        </div>
      ...
      {/* ============ DATATABLE ============ */}
      <Section title="DataTable" description="TanStack v8 wrapper. Always pass column id + accessorKey/Fn + cell render fn.">
      ...
      {/* ============ FILTER BAR ============ */}
      <Section title="FilterBar" description="The filter row above tables. Pass active filters for the chip dismiss row.">
        <FilterBar
          active={
            filterStatus
              ? [{ key: 'status', label: `Status: ${filterStatus}`, onClear: () => setFilterStatus('') }]
              : []
          }
      ...
      {/* ============ TABS ============ */}
      <Section title="Tabs" description="Headless UI Tab.Group under the hood. Pass an items array.">
        <Tabs
          tabs={[
            {
              label: 'Overview',
      ...
      {/* ============ OVERLAYS ============ */}
      <Section title="Overlays" description="Dialog for confirmations; Drawer for record detail; Tooltip for hint copy.">
        <div className="flex flex-wrap gap-2">
        ...
        >
          <p className="text-body text-surface-700">
            Type <code className="font-mono text-xs bg-surface-100 px-1 py-0.5 rounded">CR-042</code> below
            to confirm.
          </p>
        </Dialog>
        ...
        >
          <div className="space-y-4">
            ...
            <p className="text-body text-surface-800">
              Drawers are the canonical pattern for record detail. They preserve list context, support
              nesting (HUI stack), and accept a footer.
            </p>
            <Card density="cozy">
      ...
      {/* ============ EMPTY / SKELETON ============ */}
      <Section title="EmptyState &amp; Skeleton" description="Always render a meaningful loading and empty state. No bare spinners.">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card>
            <EmptyState
              icon={<Search />}
      ...
      {/* ============ MISC ============ */}
      <Section title="Miscellaneous" description="Keyboard hints, breadcrumbs (top-level via Layout), and other small bits.">
      ...
      <Section title="Rules of the road">
        <Card density="cozy">
          <ul className="space-y-2 text-small text-surface-800">
            <Rule>Never use raw <code className="font-mono text-xs">gray-*</code> classes — only <code className="font-mono text-xs">surface-*</code>.</Rule>
            <Rule>Page bg is <code className="font-mono text-xs">surface-50</code>, card bg is white, borders are <code className="font-mono text-xs">surface-200</code>.</Rule>
            <Rule>Brand emerald is for primary CTAs and positive/passing semantics; never for accent decoration.</Rule>
            <Rule>Accent terracotta is used sparingly for emphasis and human moments — never for status.</Rule>
            <Rule>Status colors: success = emerald, warning = amber, danger = red. Always use <code className="font-mono text-xs">Badge variant</code>, never custom.</Rule>
            <Rule><strong>Always use the Badge primitive (or <code className="font-mono text-xs">categoryChipClass</code>) for pills.</strong> Never hand-roll <code className="font-mono text-xs">px-2 py-1 rounded</code> spans — they drift. Search the codebase before adding new pill markup.</Rule>
            <Rule>Badge capitalizes its content by default. Pass <code className="font-mono text-xs">capitalize={'{false}'}</code> only for acronyms (PDF, MFA, CSV) or HTTP methods (GET, POST).</Rule>
            <Rule>Enum values are snake_case in the API. Render them with <code className="font-mono text-xs">{'{value.replace(/_/g, " ")}'}</code> inside a Badge — capitalize is on by default so the result is Title Case.</Rule>
            <Rule>For per-category/per-domain pills (control category, framework type, permission resource), use <code className="font-mono text-xs">categoryChipClass(value)</code> from <code className="font-mono text-xs">@/lib/categoryStyle</code> — never a custom color map.</Rule>
            <Rule>For risk status pills, use <code className="font-mono text-xs">riskStatusVariant(status)</code> from <code className="font-mono text-xs">@/lib/riskStatus</code> as the Badge variant. Single source of truth across Drawer, Detail, Dashboard.</Rule>
            <Rule>Use 700/800 text shades on light bg (<code className="font-mono text-xs">text-emerald-700</code>, <code className="font-mono text-xs">text-red-700</code>, <code className="font-mono text-xs">text-amber-800</code>). Avoid 300/400 — those were dark-mode defaults and are unreadable on cream.</Rule>
            <Rule>Pills use <code className="font-mono text-xs">rounded-md</code>. <code className="font-mono text-xs">rounded-full</code> is for avatars and dots only — never status pills.</Rule>
            <Rule>Every list page: <code className="font-mono text-xs">PageHeader</code> → <code className="font-mono text-xs">FilterBar</code> → <code className="font-mono text-xs">DataTable</code> (or Card grid).</Rule>
            <Rule>Every detail page: back link → <code className="font-mono text-xs">PageHeader</code> with meta + actions → meta strip → <code className="font-mono text-xs">Tabs</code>.</Rule>
            <Rule>Drawer-first: list row click opens a Drawer, not a navigation. Open full page is a footer action.</Rule>
            <Rule>Always render a <code className="font-mono text-xs">Skeleton</code> while loading and an <code className="font-mono text-xs">EmptyState</code> when there's nothing.</Rule>
            <Rule>Icons from <code className="font-mono text-xs">lucide-react</code> first; <code className="font-mono text-xs">@heroicons/react/24/outline</code> only for legacy parity.</Rule>
          </ul>
        </Card>
      </Section>
    </div>
...
function Subsection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">{title}</p>
      {children}
    </div>
  );
}
