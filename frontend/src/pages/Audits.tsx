...
export default function Audits() {
  ...
  if (statusFilter) {
    ...
    activeFilters.push({ key: 'status', label: `Status: ${l}`, onClear: () => setStatusFilter('') });
  }
  if (typeFilter) {
          ...
          <EmptyState
            ...
            action={
              activeFilters.length ? (
                ...
                <Link to="/audits/new">
                  <Button size="sm" leftIcon={<Plus className="h-4 w-4" />}>New Audit</Button>
                </Link>
              )
            }
          />
            ...
            >
              <CardBody density="comfy">
                <div className="flex items-start justify-between mb-4 gap-4">
                  ...
                  <Badge variant={STATUS_VARIANT[audit.status] ?? 'neutral'} dot className="capitalize shrink-0">
                    {audit.status}
                  </Badge>
                </div>
