...
export default function ReportBuilder() {
  ...
  const canSave =
    !!config.templateId && !!config.name.trim() && !!config.format && !!config.schedule;
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="rounded-md bg-brand-500/10 p-2 text-brand-700">{t.icon}</div>
                  {selected && <CheckCircle2 className="h-4 w-4 text-brand-700" />}
                </div>
