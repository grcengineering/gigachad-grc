  ...
  const columns: DataTableColumn<ScheduledReport>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (list.length === 0) return <span className="text-surface-500">—</span>;
        return (
            ...
            <div className="flex items-center justify-between gap-2 w-full">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  loading={runMutation.isPending && runMutation.variables === detailReport.id}
          ...
          <div className="space-y-5">
              ...
              {detailReport.recipients && detailReport.recipients.length > 0 ? (
                <ul className="space-y-1">
                  {detailReport.recipients.map((r) => (
                    <li key={r} className="text-small text-surface-800 font-mono">
                      {r}
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-small text-surface-500">No recipients configured.</p>
              )}
            </div>
            ...
            <div>
              ...
              {detailReport.runs && detailReport.runs.length > 0 ? (
                ...
                <p className="text-small text-surface-500">This report has not run yet.</p>
              )}
            </div>
          </div>
        )}
