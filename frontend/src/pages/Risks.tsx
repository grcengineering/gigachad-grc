...
export default function Risks() {
  ...
  if (filters.riskLevel) {
    const l =
      RISK_LEVEL_OPTS.find((o) => o.value === filters.riskLevel)?.label ?? filters.riskLevel;
    activeFilters.push({
      key: 'riskLevel',
      label: `Level: ${l}`,
      onClear: () => updateFilter('riskLevel', ''),
    });
  }
    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.residualRisk ? (
          <div className="flex items-center gap-2">
            <span
              className={`h-2 w-2 rounded-full ${RISK_LEVEL_DOT[row.original.residualRisk] || 'bg-surface-300'}`}
            />
            <span className="capitalize text-surface-700">
              {row.original.residualRisk.replace(/_/g, ' ')}
            </span>
          </div>
        ) : (
          ...
          <StatCard
            ...
            value={
              dashboard.byStatus?.find(
                (s: { status: string; count: number }) => s.status === 'mitigated'
              )?.count ?? 0
            }
            tone="emerald"
            active={filters.status === 'mitigated'}
            onClick={() =>
              updateFilter('status', filters.status === 'mitigated' ? '' : 'mitigated')
            }
          />
        </div>
      )}
