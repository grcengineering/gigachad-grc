...
import { PlusIcon, BookOpenIcon, MagnifyingGlassIcon, ArrowUpTrayIcon, ArrowDownTrayIcon } from '@heroicons/react/24/outline';
...
export default function KnowledgeBase() {
  ...
  const { data: entries = [], isLoading: loading } = useQuery({
    queryKey: ['knowledge-base', category, search],
...
privacy,GDPR Compliance,Are you GDPR compliant?,"We are fully GDPR compliant and maintain all necessary documentation, including DPIAs and data mapping.",GDPR;privacy;compliance,GDPR,approved,true`;
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_URL || ''}/knowledge-base/bulk`,
        { entries: parsedEntries },
        { withCredentials: true }
      );
      const result = response.data;

      alert(`Successfully uploaded ${result.success} entries. ${result.failed > 0 ? `Failed: ${result.failed}` : ''}`);
      ...
      alert('Failed to upload entries. Please check the CSV format.');
    } finally {
      setUploading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-surface-400">Loading knowledge base...</div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {entries.length === 0 ? (
        <div className="bg-surface-900 border border-surface-800 rounded-lg p-12 text-center">
          <BookOpenIcon className="w-12 h-12 text-surface-600 mx-auto mb-4" />
          <h3 className="text-lg font-medium text-surface-300 mb-2">No entries found</h3>
          <p className="text-surface-500 mb-6">Start building your knowledge base</p>
          <button
            onClick={() => navigate('/knowledge-base/new')}
