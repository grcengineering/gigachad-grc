...
export default function KnowledgeBase() {
  ...
  if (category)
    activeFilters.push({ key: 'category', label: `Category: ${category}`, onClear: () => setCategory('') });
  const clearAll = () => {
    setSearch('');
    setCategory('');
  };
      ...
      {isLoading ? (
        ...
        <Card>
          <EmptyState
            ...
            action={
              <Button size="sm" leftIcon={<Plus className="h-4 w-4" />} onClick={() => navigate('/knowledge-base/new')}>
                New Entry
              </Button>
            }
          />
              ...
              <CardBody density="comfy">
                <div className="flex items-start justify-between mb-2 gap-3">
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    <Badge variant={STATUS_VARIANT[entry.status] ?? 'neutral'} size="sm" className="capitalize">
                      {entry.status}
                    </Badge>
                    {entry.isPublic && (
      ...
      <div className="space-y-4">
        ...
        <div className="border-2 border-dashed border-surface-300 rounded-md p-8 text-center">
          <input
            ...
            id="kb-csv-upload"
          />
          <label htmlFor="kb-csv-upload" className="cursor-pointer inline-flex flex-col items-center">
