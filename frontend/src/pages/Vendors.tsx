...
export default function Vendors() {
  ...
  const columns: DataTableColumn<Vendor>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.inherentRiskScore ? (
          <Badge variant={RISK_VARIANT[row.original.inherentRiskScore] ?? 'neutral'} dot className="capitalize">
            {row.original.inherentRiskScore}
          </Badge>
        ) : (
    ...
    },
    {
      ...
      mobileLabel: 'Status',
      cell: ({ row }) => (
        <Badge variant={STATUS_VARIANT[row.original.status] ?? 'warning'} dot className="capitalize">
          {row.original.status}
        </Badge>
      ),
    },
      ...
      <DataTable
        ...
        emptyState={
          <EmptyState
            ...
            action={
              <Button size="sm" leftIcon={<Plus className="h-4 w-4" />} onClick={() => navigate('/vendors/new')}>
                Add Vendor
              </Button>
            }
          />
