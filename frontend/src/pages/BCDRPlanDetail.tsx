...
export default function BCDRPlanDetail() {
  ...
  const { data: plan, isLoading, isError } = useQuery<BCDRPlanDetailData>({
    ...
    return (
      <div className="space-y-5 animate-fade-in">
        <div className="flex items-center gap-2">
          <Link to="/bcdr/plans" className="inline-flex items-center gap-1 text-small text-surface-600 hover:text-surface-900">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {deactivationCriteria && (
                <div>
                  ...
                  <p className="text-surface-800 whitespace-pre-wrap mt-1">{deactivationCriteria}</p>
                </div>
              )}
            </div>
          </CardBody>
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-surface-500 font-mono">{getProcessCode(process)}</p>
                    </div>
                  </div>
        ...
        {procedures.length === 0 ? (
          ...
          <ol className="space-y-3">
            {procedures.map((procedure, idx) => (
              <li key={procedure.id} className="flex gap-3 p-3 rounded-md bg-surface-50 border border-surface-200">
      ...
      <Card>
        <CardBody density="comfy">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              <p className="text-xs text-surface-500 uppercase tracking-wider font-medium">Effective Date</p>
              <p className="text-surface-900 mt-1 tabular-nums">{formatDate(effectiveDate)}</p>
            </div>
            <div>
              <p className="text-xs text-surface-500 uppercase tracking-wider font-medium">Review Frequency</p>
              <p className="text-surface-900 mt-1 capitalize">
                {reviewFrequency ? reviewFrequency.replace(/_/g, ' ') : '—'}
              </p>
            </div>
            <div>
              <p className="text-xs text-surface-500 uppercase tracking-wider font-medium">Next Review</p>
