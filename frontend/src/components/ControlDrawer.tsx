...
interface ControlDetail {
  ...
  implementations?: Array<{
    ...
    tests?: Array<{ id: string; result: string; testType: string; testedAt: string; findings?: string }>;
  }>;
  mappings?: Array<{
    id: string;
    mappingType: string;
...
export function ControlDrawer({ control, open, onClose }: ControlDrawerProps) {
  ...
  const status: Status = (impl?.status || control.implementation?.status || 'not_started') as Status;
      ...
      <div className="space-y-6">
        ...
        {description && (
          <Section title="Description">
            <p className="text-body text-surface-800 whitespace-pre-wrap leading-relaxed">{description}</p>
          </Section>
        )}
          ...
          {detailLoading && !impl ? (
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-3 rounded-md border border-surface-200 bg-surface-50/60 p-3">
              ...
              <MetaField
                ...
                value={<span className="capitalize">{(impl.testingFrequency || 'quarterly').replace(/_/g, ' ')}</span>}
              />
              <MetaField
                icon={<Calendar className="h-3.5 w-3.5" />}
                label="Last tested"
                value={impl.lastTestedAt ? new Date(impl.lastTestedAt).toLocaleDateString() : 'Never'}
              />
              <MetaField
                icon={<Calendar className="h-3.5 w-3.5" />}
                label="Next due"
                value={
                  impl.nextTestDue ? (
                    <span className={new Date(impl.nextTestDue) < new Date() ? 'text-red-600 font-medium' : ''}>
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-small font-medium text-brand-700">{m.framework.name}</span>
                    ...
                    <p className="text-xs text-surface-500 truncate mt-0.5">
                      <span className="font-mono">{m.requirement.reference}</span> · {m.requirement.title}
                    </p>
                  </div>
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-small text-surface-800 font-medium truncate">{link.evidence.title}</p>
                      <p className="text-xs text-surface-500 capitalize truncate">{link.evidence.type}</p>
                    </div>
                  </div>
                  <Badge variant={evidenceStatusVariant(link.evidence.status)} size="sm" className="shrink-0 capitalize">
                  ...
                  link.policy ? (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-small text-surface-800 font-medium truncate">{link.policy.title}</p>
                          <p className="text-xs text-surface-500 capitalize truncate">
                            {link.policy.category?.replace(/_/g, ' ')} · v{link.policy.version}
                          </p>
                        </div>
                      </div>
                      <Badge variant={policyStatusVariant(link.policy.status)} size="sm" className="shrink-0 capitalize">
                        {link.policy.status}
                      </Badge>
                    </Link>
                  ) : null,
              ...
              {recentTests.map((test) => (
                ...
                >
                  <div className="flex items-start gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <Badge variant={testResultVariant(test.result)} size="sm" className="capitalize">
                          {test.result}
                        </Badge>
                        <span className="text-xs text-surface-500 capitalize">{test.testType} test</span>
                      </div>
                      {test.findings && (
                        <p className="text-xs text-surface-600 mt-1 line-clamp-2">{test.findings}</p>
                      )}
                    </div>
                  </div>
        ...
        {guidance && (
          <Section title="Implementation Guidance">
            <p className="text-small text-surface-700 whitespace-pre-wrap leading-relaxed">{guidance}</p>
          </Section>
        )}
