      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {risk.status ? (
            ...
            <Badge variant="neutral" size="md">No status</Badge>
          )}
        </div>
        ...
        {/* Relationships */}
        {(risk.assetCount !== undefined || risk.controlCount !== undefined || risk.scenarioCount !== undefined) && (
          <div>
            <h3 className="text-xs font-medium text-surface-500 uppercase tracking-wider mb-2">
              Relationships
            </h3>
  ...
  return (
    <div className="rounded-md border border-surface-200 bg-white/40 p-3">
      ...
      <div className="flex items-center gap-2">
        <span className={cn('h-2.5 w-2.5 rounded-full', RISK_LEVEL_DOT[level] || 'bg-surface-500')} />
        <span className="text-body text-surface-900 capitalize font-medium">{String(level).replace(/_/g, ' ')}</span>
      </div>
    </div>
  );
}
