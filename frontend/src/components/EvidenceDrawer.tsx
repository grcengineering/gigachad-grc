      ...
      {isLoading || !evidence ? (
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Badge
              variant={STATUS_VARIANT[evidence.status] ?? 'neutral'}
              dot
              size="md"
            ...
            <Section title="Review">
              <div className="rounded-md border border-surface-200 bg-surface-50/60 p-3 space-y-2">
                ...
                {evidence.reviewNotes && (
                  <p className="text-small text-surface-700 whitespace-pre-wrap">
                    {evidence.reviewNotes}
                  </p>
                )}
              </div>
                ...
                {evidence.controlLinks.slice(0, 6).map((link) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-small text-surface-800 truncate mt-0.5">
                        {link.control.title}
                      </p>
                    </div>
...
function getFileKind(mime?: string): 'image' | 'pdf' | 'text' | 'spreadsheet' | 'doc' | 'other' {
  ...
  if (
    ...
    ['application/json', 'application/xml', 'application/javascript', 'application/yaml'].includes(
      mime
    )
  ) {
    return 'text';
