...
export type StatCardTone =
  'brand' | 'accent' | 'emerald' | 'red' | 'amber' | 'blue' | 'purple' | 'neutral';
...
> = {
  brand: {
    iconBg: 'bg-brand-500/10 text-brand-700 dark:bg-brand-500/20 dark:text-brand-300',
    spark: { stroke: 'rgb(16 185 129)', fill: 'rgba(16, 185, 129, 0.15)' },
  },
  accent: {
    iconBg: 'bg-accent-500/10 text-accent-700 dark:bg-accent-500/20 dark:text-accent-300',
  ...
  neutral: {
    iconBg: 'bg-surface-100 text-surface-700 dark:bg-surface-800 dark:text-surface-300',
      ...
      <CardBody density="comfy">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-xs text-surface-500 dark:text-surface-400 uppercase tracking-wider font-medium">
              {label}
            </p>
            <p className="text-display text-surface-900 dark:text-surface-100 mt-1 tabular-nums">
              {value}
            </p>
            {caption && (
              <p className="text-xs text-surface-500 dark:text-surface-400 mt-0.5">{caption}</p>
            )}
            {delta !== undefined && (
