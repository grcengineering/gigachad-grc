...
export class SeedDataService {
  ...
  private async renderDemoPdf(title: string, description: string): Promise<Buffer> {
    ...
    document.moveDown();
    document.fontSize(11).text(description);
    document.moveDown();
    document.fontSize(9).fillColor('#666666').text('Generated by GigaChad GRC demo data.');
    document.end();
  ...
  private async seedEvidence(organizationId: string, userId: string): Promise<string[]> {
    ...
    for (let i = 0; i < evidenceItems.length; i++) {
      ...
      const extension = isScreenshot ? 'png' : 'pdf';
      const filename = `${item.title.toLowerCase().replace(/[^a-z0-9]/g, '_')}.${extension}`;
      const storagePath = `evidence/${organizationId}/${Date.now()}-${i}.${extension}`;
      const contents = isScreenshot
        ? Buffer.from(
            'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=',
            'base64'
          )
        : await this.renderDemoPdf(item.title, item.description);
      await this.storage.upload(contents, storagePath, {
        contentType: isScreenshot ? 'image/png' : 'application/pdf',
      });
      ...
      const evidence = await this.prisma.evidence.create({
        data: {
          ...
          filename,
          mimeType: isScreenshot ? 'image/png' : 'application/pdf',
  ...
  private async seedPolicies(organizationId: string, userId: string): Promise<number> {
    ...
    for (let index = 0; index < DEMO_POLICIES.length; index++) {
      const policy = DEMO_POLICIES[index];
      const filename = `${policy.title.toLowerCase().replace(/[^a-z0-9]/g, '_')}.pdf`;
      const storagePath = `policies/${organizationId}/${Date.now()}-${index}.pdf`;
      const contents = await this.renderDemoPdf(policy.title, policy.description);
      await this.storage.upload(contents, storagePath, {
