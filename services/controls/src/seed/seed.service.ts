...
export class SeedDataService {
  ...
  async loadDemoData(organizationId: string, userId: string): Promise<SeedResult> {
    ...
    try {
      ...
      const { frameworkIds, requirementIds } = await this.seedFrameworksWithRequirements(
        organizationId,
        userId
      );
      ...
      const { controlIds, implementationIds } = await this.seedControlsWithImplementations(
        organizationId,
        userId
      );
      ...
      result.recordsCreated.controlMappings = await this.seedControlMappings(
        organizationId,
        controlIds,
        requirementIds
      );
      ...
      result.recordsCreated.evidenceLinks = await this.seedEvidenceControlLinks(
        organizationId,
        evidenceIds,
        controlIds,
        implementationIds,
      ...
      const { vendorIds, assessmentCount } = await this.seedVendorsWithAssessments(
        organizationId,
        userId
      );
      ...
      result.recordsCreated.trainingRecords = await this.seedTrainingRecords(
        organizationId,
        employeeIds
      );

      // Create background checks for employees
      result.recordsCreated.backgroundChecks = await this.seedBackgroundChecks(
        organizationId,
        employeeIds
      );
  ...
  private async seedFrameworksWithRequirements(
    organizationId: string,
    _userId: string
  ): Promise<{ frameworkIds: string[]; requirementIds: string[] }> {
    ...
    for (const catalogId of catalogFrameworkIds) {
      ...
      this.logger.log(
        `Seeded framework '${catalogFramework.name}' with ${flatRequirements.length} requirements`
      );
    }
  ...
  private async seedControlsWithImplementations(
    organizationId: string,
    userId: string
  ): Promise<{ controlIds: string[]; implementationIds: string[] }> {
        ...
        data: {
          ...
          lastTestedAt:
            status === 'implemented'
              ...
              : null,
          nextTestDue:
            status === 'implemented' ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) : null,
          effectivenessScore:
            status === 'implemented'
  ...
  private async seedControlMappings(
    organizationId: string,
    controlIds: string[],
  ...
  private async seedEvidence(organizationId: string, userId: string): Promise<string[]> {
    ...
    const evidenceItems = [
      {
        title: 'AWS Security Hub Report',
        type: 'report',
        source: 'aws',
        category: 'Cloud Security',
        description: 'Monthly AWS Security Hub findings and compliance status',
      },
      {
        ...
        description: 'External penetration test results from third-party assessor',
      },
      {
        title: 'Access Review Export - November 2024',
        type: 'export',
        source: 'okta',
        category: 'Access Control',
        description: 'Quarterly access review completed for all systems',
      },
      {
        ...
        source: 'manual',
        category: 'Compliance',
        description: 'Internal readiness assessment for SOC 2 certification',
      },
      {
        ...
        description: 'Weekly vulnerability scan from Datadog Security Monitoring',
      },
      {
        title: 'Employee Security Training Completion',
        type: 'export',
        source: 'knowbe4',
        category: 'Security Awareness',
        description: 'Training completion report for security awareness program',
      },
      {
        title: 'GitHub Branch Protection Settings',
        type: 'screenshot',
        source: 'github',
        category: 'Development',
        description: 'Screenshot of branch protection rules on main repositories',
      },
      {
        ...
        description: 'Export of firewall rules and configuration',
      },
      {
        ...
        description: 'Current incident response procedures document',
      },
      {
        ...
        description: 'Data classification guidelines and handling procedures',
      },
      {
        ...
        source: 'okta',
        category: 'Access Control',
        description: 'Multi-factor authentication enrollment status for all users',
      },
      {
        ...
        description: 'Backup restoration test results and verification',
      },
      {
        ...
        description: 'Change advisory board approvals and deployment history',
      },
      {
        title: 'SSL Certificate Inventory',
        type: 'export',
        source: 'manual',
        category: 'Network Security',
        description: 'Inventory of all SSL/TLS certificates and expiration dates',
      },
      {
        ...
        description: 'Complete inventory of managed devices and software',
      },
      {
        ...
        source: 'aws',
        category: 'Data Protection',
        description: 'AWS KMS configuration and encryption settings',
      },
      {
        ...
        description: 'Annual physical security assessment of office locations',
      },
      {
        title: 'Vendor Security Questionnaire - AWS',
        type: 'document',
        source: 'manual',
        category: 'Third Party',
        description: 'Completed security questionnaire from AWS',
      },
      {
        ...
        source: 'manual',
        category: 'Network Security',
        description: 'Current network architecture and segmentation diagram',
      },
      {
        ...
        source: 'okta',
        category: 'Access Control',
        description: 'Audit of privileged access accounts and permissions',
      },
    ];
  ...
  private async seedVendorsWithAssessments(
    organizationId: string,
    userId: string
  ): Promise<{ vendorIds: string[]; assessmentCount: number }> {
        ...
        data: {
          ...
          category:
            vendor.category === 'Cloud Infrastructure'
              ...
              : vendor.category === 'CRM'
                ? 'software_vendor'
                : vendor.category === 'Collaboration'
                  ? 'software_vendor'
                  : vendor.category === 'Development'
                    ? 'software_vendor'
                              ...
                              : vendor.category === 'Legal'
                                ...
                                : vendor.category === 'HR'
                                  ...
                                  : vendor.category === 'Security'
                                    ? 'software_vendor'
                                    : vendor.category === 'Infrastructure'
                                      ? 'cloud_provider'
                                      : vendor.category === 'Communications'
                                        ? 'software_vendor'
                                        : vendor.category === 'Data Warehouse'
                                          ? 'cloud_provider'
  ...
  private async seedTrainingRecords(
    organizationId: string,
    employeeIds: string[]
  ): Promise<number> {
    let count = 0;
    ...
    for (const employeeId of employeeIds) {
      ...
      const courses = [...DEMO_TRAINING_COURSES]
        .sort(() => 0.5 - Math.random())
      ...
      for (const course of courses) {
        ...
        await this.prisma.employeeTrainingRecord.create({
          data: {
            ...
            completedAt: isCompleted
              ? this.randomDate(new Date('2024-11-01'), new Date('2025-01-15'))
              : null,
  ...
  private async seedBackgroundChecks(
    organizationId: string,
    employeeIds: string[]
  ): Promise<number> {
    let count = 0;
    ...
    for (const employeeId of employeeIds) {
      ...
      if (Math.random() > 0.1) {
        ...
        const completedAt =
          status === 'clear'
            ? this.randomDate(new Date('2023-01-01'), new Date('2024-12-01'))
            : null;
        ...
        await this.prisma.employeeBackgroundCheck.create({
          data: {
            ...
            expiresAt: completedAt
              ? new Date(completedAt.getTime() + 365 * 3 * 24 * 60 * 60 * 1000)
              : null, // 3 years
          },
        });
        count++;
    ...
    for (const audit of DEMO_AUDITS) {
      await this.prisma.audit.create({
        data: {
          ...
          framework:
            audit.type === 'soc2_type2' ? 'SOC2' : audit.type === 'iso27001' ? 'ISO27001' : null,
          createdBy: userId,
        },
      });
      count++;
  ...
  private async seedAuditFindings(organizationId: string, userId: string): Promise<number> {
    ...
    const findingTemplates = [
      {
        title: 'Access controls not properly documented',
        description: 'Access control procedures lack formal documentation and approval workflows.',
      },
      {
        title: 'Encryption key rotation not implemented',
        description: 'Cryptographic keys are not rotated according to policy requirements.',
      },
      {
        title: 'Vulnerability scanning gaps',
        description: 'Critical systems are not included in the vulnerability scanning scope.',
      },
      {
        title: 'Incident response testing overdue',
        description: 'Annual incident response tabletop exercise has not been conducted.',
      },
      {
        title: 'Third-party risk assessment incomplete',
        description: 'Critical vendors have not undergone annual security assessments.',
      },
      {
        title: 'Audit log retention insufficient',
        description: 'Audit logs are being purged before the required retention period.',
      },
      {
        title: 'MFA not enforced for privileged accounts',
        description: 'Some administrator accounts can bypass multi-factor authentication.',
      },
      {
        title: 'Change management exceptions undocumented',
        description: 'Emergency changes lack proper documentation and approval.',
      },
    ];
    ...
    for (const audit of audits) {
      ...
      const selectedTemplates = findingTemplates
        .sort(() => 0.5 - Math.random())
        .slice(0, numFindings);
      ...
      for (const template of selectedTemplates) {
        ...
        await this.prisma.auditFinding.create({
          data: {
            ...
            targetDate:
              status !== 'resolved'
                ? this.randomDate(new Date('2025-02-01'), new Date('2025-06-30'))
                : null,
  ...
  private async seedBCDRData(
    organizationId: string,
    userId: string
  ): Promise<{ processes: number; plans: number; tests: number }> {
    ...
    try {
      ...
      const processData = [
        ...
        {
          name: 'Internal Wiki',
          criticalityTier: 'tier_4_deferrable',
          rtoHours: 72,
          rpoHours: 48,
          department: 'IT',
        },
      ...
      const planData = [
        {
          title: 'Enterprise Business Continuity Plan',
      ...
      const testData = [
        {
          name: 'Q4 2024 Tabletop Exercise',
          testType: 'tabletop',
          status: 'completed',
          result: 'passed',
        },
        {
          name: 'Q1 2025 Failover Test',
          testType: 'functional',
          status: 'completed',
      ...
      this.logger.log(
        `Seeded BC/DR data: ${result.processes} processes, ${result.plans} plans, ${result.tests} tests`
      );
    } catch (error) {
