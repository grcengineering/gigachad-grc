...
export class RiskService {
  ...
  async getDashboard(organizationId: string): Promise<RiskDashboardDto> {
    const [
      ...
      this.prisma.risk.count({
        where: {
          organizationId,
          deletedAt: null,
          status: { in: [RiskIntakeStatus.RISK_IDENTIFIED, RiskIntakeStatus.ACTUAL_RISK, RiskIntakeStatus.RISK_ANALYSIS_IN_PROGRESS, RiskIntakeStatus.RISK_ANALYZED] },
        },
      }),
      ...
      this.prisma.risk.findMany({
        where: {
          ...
          deletedAt: null,
          treatment: {
            mitigationTargetDate: { lt: new Date() },
            status: { notIn: [RiskTreatmentStatus.RISK_MITIGATION_COMPLETE, RiskTreatmentStatus.RISK_ACCEPT, RiskTreatmentStatus.RISK_AVOID, RiskTreatmentStatus.RISK_TRANSFER] },
          },
        },
      ...
      this.prisma.riskAssessment.count({
        where: {
          ...
          status: { in: [RiskAssessmentStatus.RISK_ASSESSOR_ANALYSIS, RiskAssessmentStatus.GRC_APPROVAL, RiskAssessmentStatus.GRC_REVISION] },
        },
      }),
      this.prisma.riskTreatment.count({
        where: {
          risk: { organizationId, deletedAt: null },
          status: RiskTreatmentStatus.TREATMENT_DECISION_REVIEW,
        },
      }),
      this.prisma.riskTreatment.count({
        where: {
          risk: { organizationId, deletedAt: null },
          status: { in: [RiskTreatmentStatus.IDENTIFY_EXECUTIVE_APPROVER, RiskTreatmentStatus.EXECUTIVE_APPROVAL] },
        },
      }),
      this.prisma.riskTreatment.count({
        where: {
          risk: { organizationId, deletedAt: null },
          status: RiskTreatmentStatus.RISK_MITIGATION_IN_PROGRESS,
        },
      }),
    ]);
    ...
    return {
      ...
      byCategory: byCategory.map(c => ({ category: c.category || 'uncategorized', count: c._count._all })),
      byRiskLevel: byRiskLevel.map(r => ({ level: r.inherentRisk || 'unknown', count: r._count._all })),
