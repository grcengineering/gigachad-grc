...
const trustServiceCriteria: Record<
  string,
  { description: string; controlPoints: { id: string; name: string; description: string }[] }
> = {
  security: {
...
export async function checkSOC2Controls(params: SOC2CheckParams): Promise<SOC2CheckResult> {
  ...
  for (const category of trustServiceCategories) {
    ...
    const avgCategoryScore =
      controlsToCheck.length > 0 ? Math.round(categoryScore / controlsToCheck.length) : 0;
    ...
    categories.push({
      ...
      status:
        avgCategoryScore >= 80 ? 'Compliant' : avgCategoryScore >= 50 ? 'Partial' : 'Non-Compliant',
  ...
  if (criticalFindings.length > 0) {
    recommendations.push(
      `URGENT: Address ${criticalFindings.length} critical finding(s) immediately`
    );
  }
