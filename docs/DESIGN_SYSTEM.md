...
As of the latest snapshot, ESLint reports **~521 violations** across the codebase. These are pre-existing patterns from before the primitives existed. The pre-commit hook prevents _new_ violations; the backlog is paid down opportunistically when files are touched.
