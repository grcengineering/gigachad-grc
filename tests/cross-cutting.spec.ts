...
test.describe('Breadcrumbs', () => {
  test('detail page shows breadcrumbs with the entity type', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /open full page/i }).first().click();
...
test.describe('Keyboard shortcuts modal', () => {
  test('? key toggles the shortcuts modal', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /keyboard shortcuts/i })).toBeVisible({ timeout: 3_000 });
...
test.describe('Notification bell', () => {
  test('clicking the bell does not crash', async ({ page }) => {
    ...
    const bell = page.locator('button:has(svg.lucide-bell), button:has([data-icon="bell"]), button[aria-label*="notification" i]').first();
