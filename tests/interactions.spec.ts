...
async function waitForDrawerWithTitle(page: Page, titleRegex: RegExp) {
  await expect(page.getByRole('heading', { name: titleRegex }).first()).toBeVisible({ timeout: 8_000 });
}
...
test.describe('Controls — drawer-first flow', () => {
  test('clicking a row opens the drawer with the control detail', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /^Implementation$/i }).first()).toBeVisible({ timeout: 8_000 });

    // Close via X button (aria-label="Close")
    await page.getByRole('button', { name: 'Close' }).first().click();
    await expect(page.getByRole('heading', { name: new RegExp(controlIdText) })).toHaveCount(0, { timeout: 3_000 });
  ...
  test('drawer has Open Full Page button that navigates', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /open full page/i })).toBeVisible({ timeout: 8_000 });
...
test.describe('Controls — nested EvidenceDrawer (the bug we just fixed)', () => {
  test('opening evidence from inside ControlDrawer does NOT close the control drawer when evidence is closed', async ({ page }) => {
    const errs = trackPageErrors(page);
    await page.goto('/controls');
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      await page.getByRole('heading', { name: /^Evidence/i }).first().waitFor({ timeout: 8_000 }).catch(() => {});

      // Evidence rows are buttons whose accessible name starts with the evidence title and ends with a status word
      const candidates = page.getByRole('button').filter({ hasText: /(approved|pending|rejected|expired)/i });
    ...
    const evidenceHeading = page.getByRole('heading', { level: 2, name: new RegExp(evidenceTitleStart, 'i') });
    ...
    await expect(
      page.getByRole('heading', { level: 2, name: new RegExp(controlIdPrefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')) }).first(),
...
test.describe('Filter bar — Controls', () => {
  test('selecting a status filter shows an active chip; clearing it removes the chip', async ({ page }) => {
    const errs = trackPageErrors(page);
    await page.goto('/controls');
    await page.waitForLoadState('domcontentloaded');
    // Wait for filter controls to be ready
    await expect(page.getByRole('button', { name: /^All Statuses$/i })).toBeVisible({ timeout: 5_000 });
...
test.describe('Framework requirement deep-link', () => {
  test('clicking a framework mapping in ControlDrawer lands on the framework page with target requirement highlighted', async ({ page }) => {
    const errs = trackPageErrors(page);
    await page.goto('/controls');
    await page.locator('tbody tr').first().click();

    // Wait for framework mappings section
    await expect(page.getByRole('heading', { name: /^Framework Mappings/i })).toBeVisible({ timeout: 8_000 });
...
test.describe('Inline status edit in drawer', () => {
  test('changing status from the drawer Select updates the badge', async ({ page }) => {
    ...
    const newStatusName = original.toLowerCase().includes('implemented') ? /^In Progress$/i : /^Implemented$/i;
