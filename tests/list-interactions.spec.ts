...
async function openSelectAndChoose(page: Page, placeholder: string, option: RegExp) {
  // Our Select primitive renders either a Listbox.Button (whose visible text is the placeholder)
  // OR a Combobox.Input (whose placeholder attribute is the same text). Try both.
...
test.describe('Risks list', () => {
  test('Add Risk dialog opens and closes', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /create new risk/i })).toBeVisible({ timeout: 3_000 });
    await page.keyboard.press('Escape');
    await expect(page.getByRole('heading', { name: /create new risk/i })).toBeHidden({ timeout: 3_000 });
    await expectPageHealthy(page, errs);
  });
...
test.describe('Risk Queue', () => {
  test('all four tab cards are clickable and switch content', async ({ page }) => {
    ...
    for (const tab of ['My Assessments', 'Treatment Decisions', 'Executive Approvals', 'GRC Reviews']) {
...
test.describe('Risk Reports', () => {
  test('selecting each template renders its preview', async ({ page }) => {
    ...
    const templates = ['Full Risk Register', 'Risk Summary', 'Treatment Status Report', 'Risk Trend Analysis', 'Executive Summary'];
...
test.describe('Risk Scenarios', () => {
  test('clicking a scenario card opens its detail dialog', async ({ page }) => {
    ...
    await firstCard.click();
    await expect(page.getByRole('heading', { name: /phishing attack on employees/i })).toBeVisible({ timeout: 3_000 });
    await page.keyboard.press('Escape');
    await expectPageHealthy(page, errs);
  });
});
...
test.describe('Design System showcase', () => {
  test('all sections render without errors', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /delete control/i })).toBeVisible({ timeout: 2_000 });
    await page.keyboard.press('Escape');
    await expectPageHealthy(page, errs);
  });
});
