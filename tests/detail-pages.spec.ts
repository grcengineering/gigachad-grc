...
const detailRoutes: Array<{
  ...
  { name: 'QuestionnaireDetail', path: (id) => `/questionnaires/${id}`, fetchId: firstQuestionnaireId },
  { name: 'KnowledgeBaseDetail', path: (id) => `/knowledge-base/${id}`, fetchId: firstKnowledgeBaseId },
];
...
for (const route of detailRoutes) {
  test(`${route.name} loads cleanly`, async ({ page, request }) => {
    ...
    // URL didn't bounce
    await expect(page).toHaveURL(new RegExp(route.path(id).replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '$'));
